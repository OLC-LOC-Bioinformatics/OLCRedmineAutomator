...
def merge(aaastrIn, ostm, gtdb):
    ...
    for f in aaastrIn:
        ...
        #    print('merge_metaphlanestimatedreadcounts_tables: profiles from differrent versions of MetaPhlAn, please profile your '
        #          'samples using the same MetaPhlAn version.\n')
        #    return
            
        iIn = pd.read_csv(f, sep='\t', skiprows=len(headers), names=names, usecols=[0,4] if not gtdb else range(2), index_col=0)
        profiles_list.append(pd.Series(data=iIn['estimated_number_of_reads_from_the_clade'], index=iIn.index,
                                       name=os.path.splitext(os.path.basename(f))[0].replace('_profile', '')))

    merged_tables = pd.concat([merged_tables, pd.concat(profiles_list, axis=1, sort=True).fillna(0)], axis=1).fillna(0)
    #ostm.write(list(listmpaVersion)[0]+'\n')
    merged_tables.to_csv(ostm, sep='\t')


argp = argparse.ArgumentParser(prog="merge_metaphlanestimatedreadcounts_tables.py",
...
argp.usage = (argp.format_usage() + "\nPlease make sure to supply file paths to the files to combine.\n\n" +
              ...
              "   ./merge_metaphlanestimatedreadcounts_tables.py Table1.txt Table2.txt Table3.txt > output.txt\n\n" +
              "A wildcard to indicate all .txt files that start with Table can be used as follows:\n" +
              "    ./merge_metaphlanestimatedreadcounts_tables.py Table*.txt > output.txt")
...
def main( ):
    ...
    if not args.aistms:
        print('merge_metaphlanestimatedreadcounts_tables: no inputs to merge!')
        return

    if args.o and os.path.exists(args.o) and not args.overwrite:
        print('merge_metaphlanestimatedreadcounts_tables: output file "{}" exists, specify the --overwrite param to ovrewrite it!'.format(args.o))
        return
