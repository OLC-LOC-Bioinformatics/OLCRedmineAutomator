...
def unknownisolatev2_redmine(redmine_instance, issue, work_dir, description):
    ...
    try:
        ...
        # The analysis type will default to sketching against the entire database, unless custom is specified
        ...
        if not argument_dict['query']:
            redmine_instance.issue.update(resource_id=issue.id,
                                          notes='WARNING: Could not identify query sequence. '
                                                'Please ensure that the request includes one.'.format(ats=', '.join(analyses)),
                                          status_id=4)
            return
        ...
        #if a completely unknown isolate, pull the mash sketch db of all atcc and refseq assemblies for analysis
        if argument_dict['analysis'] == 'default':
            src = '/mnt/nas2/processed_sequence_data/atcc/mash_db_with_refseq/'
            fasta_dir = os.path.join(work_dir, 'fastas')
            if not os.path.isdir(fasta_dir):
                os.makedirs(fasta_dir)
            dst = fasta_dir
            lncmd = 'ln -s {src}all_references.msh {dst}'.format(src=src,
                                                      dst=dst)
            os.system(lncmd)
        ...
        if argument_dict['query'] == 'attached': 
            ...
            for assembly in glob.glob(os.path.join(work_dir, '*.fasta')):
                ...
                redmine_instance.issue.update(resource_id=issue.id,
                                              notes='ERROR: The query was listed as "attached". '
                                                    'The automator could not find any attached files. '
                                                    'Please create a new issue with the query.fasta file attached and try '
                                                    'again.',
                                              status_id=4)
        else:
        ...
        if os.stat(os.path.join(work_dir, 'output.txt')).st_size == 0:
            ...
            #parse the pubmlst results... there is probably a better way to do this
            rank_line = [0]
            taxon_line = [1]
            support_line = [2]
            taxonomy_line = [3]
        ...
        #Run a mash to figure out if any strains are particularly far apart... this matters more for custom analysis
	# Make the reference file
        ...
        #Only report this if the analysis type is not default, or else we'll end up with thousands of lines in the redmine output
        if argument_dict['analysis'] != 'default':
        ...
        # Move distances.tab and sketch.msh from fastas folder
        ...
        if argument_dict['query'] == 'attached':
            ...
            #attempt 2
            #there HAS to be a better way to get the filename and copy the file? whyyy won't it behave
...
def check_distances(ref_fasta, fasta_folder):
    ...
    mash.dist(os.path.join(fasta_folder, 'all_references.msh'), ref_fasta, threads=46, output_file=os.path.join(fasta_folder, 'distances.tab'))
    mash_output = mash.read_mash_output(os.path.join(fasta_folder, 'distances.tab'))
    for item in mash_output:
        print(item.reference, item.query) #trying to troubleshoot string to float issue... this was due to spaces in the assembly names
...
def closest_distances(ref_fasta, fasta_folder):
    ...
    mash_output = mash.read_mash_output(os.path.join(fasta_folder, 'distances.tab'))
    for item in mash_output:
        print(item.reference, item.query) #trying to troubleshoot string to float issue... this was due to spacing in the assembly names
        print(item.reference, item.query, str(item.distance))
