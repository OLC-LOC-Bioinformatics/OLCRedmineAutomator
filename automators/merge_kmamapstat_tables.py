...
def merge(aaastrIn, ostm, gtdb):
    ...
    for f in aaastrIn:
        ...
        headers2 = [x.strip() for x in takewhile(lambda x: x.startswith('#'), open(f))]
        #listmpaVersion.add(headers[0])
        rowstoskip = len(headers)+1
        names = headers2[-1].split('#')[1].strip().split('\t')
        ...
        iIn = pd.read_csv(f, sep='\t', skiprows=rowstoskip, names=names, usecols=[0,1] if not gtdb else range(2), index_col=0)
        profiles_list.append(pd.Series(data=iIn['readCount'], index=iIn.index,
...
argp = argparse.ArgumentParser(prog="merge_kmamapstat_tables.py",
                               description="Performs a table join on one or more kma mapstat output files.")
...
argp.usage = (argp.format_usage() + "\nPlease make sure to supply file paths to the files to combine.\n\n" +
              ...
              "   ./merge_kmamapstat_tables.py Table1.txt Table2.txt Table3.txt > output.txt\n\n" +
              "A wildcard to indicate all .txt files that start with Table can be used as follows:\n" +
              "    ./merge_kmamapstat_tables.py Table*.txt > output.txt")
...
def main( ):
    ...
    if not args.aistms:
        print('merge_kmamapstat_tables: no inputs to merge!')
        return

    if args.o and os.path.exists(args.o) and not args.overwrite:
        print('merge_kmamapstat_tables: output file "{}" exists, specify the --overwrite param to ovrewrite it!'.format(args.o))
        return
